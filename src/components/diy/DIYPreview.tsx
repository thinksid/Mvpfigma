...
const STRIPE_BUY_BUTTON_ID = 'buy_btn_1SQCuD1epFGDXBkyTWd3p91l';
...
export const DIYPreview: React.FC<DIYPreviewProps> = ({
  ...
  const fetchGenerationData = async () => {
    try {
      ...
      // Try to get generation_id from URL query params first
      const urlParams = new URLSearchParams(window.location.search);
      const urlGenerationId = urlParams.get('id');
      const idToFetch = urlGenerationId || generationId;
      ...
      // Fetch from server
      const response = await fetch(`${SERVER_URL}/diy/${idToFetch}`, {
        method: 'GET',
        headers: {
      ...
      // Get html_code from N8N (pushed to Supabase)
      if (!data.html_code) {
      ...
      console.error('Error in fetchGenerationData:', err);
  ...
  const handleSaveHTML = async (updatedHTML: string) => {
    try {
      const urlParams = new URLSearchParams(window.location.search);
      const urlGenerationId = urlParams.get('id');
      const idToUse = urlGenerationId || generationId;

      if (!idToUse) {
        throw new Error('No generation ID found');
      }

      // Update html_code in database
      const response = await fetch(`${SERVER_URL}/diy/${idToUse}/html`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error('Error saving HTML:', errorData);
        throw new Error('Failed to save HTML changes');
      }

      // Update local state
      setHtmlCode(updatedHTML);
    } catch (err) {
      console.error('Error in handleSaveHTML:', err);
      throw err; // Re-throw to let HTMLEditor handle the error
    }
  };
