...
const PaginationLink = ({ className = '', ...props }: React.ComponentProps<'a'>) => (
  <a aria-current={props['aria-current']} className={`inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ${className}`} {...props} />
);

PaginationLink.displayName = 'PaginationLink';

const PaginationPrevious = ({ className = '', ...props }: React.ComponentProps<typeof PaginationLink>) => (
  <PaginationLink aria-label="Go to previous page" className={`gap-1 pl-2.5 ${className}`} {...props}>
